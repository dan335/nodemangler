...
- `app/crates/mangler_gui/src/panels/` — Blender-style panel system: `panel_tree.rs` (recursive split tree, pure logic), `panel_view.rs` (splitters, kind-switcher chrome), `panel_windows.rs` (secondary OS windows via immediate viewports). Each panel hosts one of 5 `PanelKind` contexts (Graph, Preview2D, Preview3D, NodeList, Settings); layout is app-level and persisted to config as `default_layout`; split/close live in each panel's own corner-button menu and act on that panel, while the settings menu keeps new-window/save-layout/reset-layout
