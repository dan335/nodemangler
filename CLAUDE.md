...
- **Batch runs** (`ChangeGraphMessage::RunBatch { node_id }` / `CancelBatch`): the engine iterates a `from folder` image-input node over its folder — one full `graph.run()` per engine tick per file, stepping the node's `index` input (echoed to the GUI via `InputChanged`) with `Graph::force_save_outputs` + `Graph::batch_item_stem` set for each iteration, then restoring the pre-batch index at the end (`BatchState`/`start_batch`/`finish_batch` in engine `app.rs`; driver and node share the deterministic listing in `from_folder::list_image_files`). Progress returns as `GraphChangedMessage::BatchProgress`/`BatchFinished`; the GUI's run-batch button + progress bar render in the from-folder node's settings panel (batch section in `node_settings_panel.rs`, per-`Program` `batch_run` state, outcome as a status message). During a batch, `resolve_output_dir_and_stem` names unwired output files per item (empty name → item stem; literal `S` → `S_{item}`; wired names used verbatim) and `to clipboard` opts out of the force flag (`honor_force` param of `should_save_and_consume`). Batch stepping never sets `needs_to_save`, so a batch alone doesn't churn the graph file's auto-save. Starting a batch that can't run (wrong node, empty/unreadable folder) reports `BatchFinished { total: 0, cancelled: true }`.
...
- **Curve** (`mangler_core/src/curve.rs`): `Value::Curve(Curve)` — a user-drawn 2D path in normalized [0,1]² y-down coords: `points: Vec<[f32;2]>` + `closed: bool` + `interpolation` (Linear | Smooth = centripetal Catmull-Rom through the points | Bezier = cubic spans with one **mirrored** tangent handle per anchor) + `handles: Vec<[f32;2]>` (`#[serde(default)]`; per-anchor tangent *offsets* — out-handle = anchor+h, in-handle = anchor−h, so anchors are C¹ by construction and moving an anchor carries its handles; entries missing/misaligned fall back to `auto_handle` = uniform Catmull-Rom tangent `(next−prev)/6`, making Smooth→Bezier shape-preserving). Plain derive-serde (points persist in graph saves, unlike images). Single subpath — use multiple curve nodes for multiple paths. **Edited as a Preview2D overlay, not in the settings panel**: when the settings-panel node (`Program.editing_node_id`) has an unconnected Curve input, every 2D preview panel overlays the curve on whatever image it shows (`mangler_gui/src/view_window/curve_overlay.rs`) — drag anchors to move, click to insert/append, double/right-click to delete (floor 2 points), in Bezier mode drag either mirrored tangent knob (registered before anchors so anchors win overlapping hits), closed/interpolation controls in a corner strip; empty-space drag still pans (egui resolves click and drag hits independently; handles are topmost drag targets). Edits mutate the local input value per-frame but send `ChangeNodeMessage::SetInput` only on gesture end, so heavy downstream nodes re-run once per drag. Settings panel/`preview_2d` show a summary via `curve_summary`; no image displayed → letterboxed square fallback canvas. **Tone-curve exception**: a Curve input marked `InputSettings::ToneCurve` is a value-mapping *function* (x = input value, y-down so top edge = output 1), not a spatial path — it's excluded from the Preview2D overlay and edited instead as an embedded Photoshop-style box in the node settings panel, one box per unconnected ToneCurve input, labeled by input name (`mangler_gui/src/settings/tone_curve_widget.rs`: source-image histogram behind a quarter grid, point x kept sorted between neighbors while dragging, same gesture-end commit pattern; the upstream image for the histogram is resolved in `program.rs` before the mutable node borrow and cached on `GraphNode.input_histogram_cache` by change_id — one cache serves all boxes on a node; no connected image → no histogram). Shared machinery lives in `operations/images/tone_curve.rs`: `tone_curve_lut` builds a 1024-entry LUT (output = 1 − y, flat clamp past the endpoint x's, x-reversed spline segments overwrite so it stays a function), `sample_lut` samples it, `optional_lut` returns None on the untouched identity default so remap-style consumers skip it and stay bit-identical, `tone_curve_input`/`identity_tone_curve`/`anti_diagonal_tone_curve` build inputs/defaults. Consumers beyond the adjustments `curves` node: falloff/remap/profile curves on inner_glow, outer_glow, vignette, distance, curve_distance_field, gradient (easing), cone/pyramid/paraboloid (height profile, anti-diagonal default = exact old linear ramp), rolling_hills + guided_rolling_hills (hill dome profile), and the `valley profile` cross-sections that replaced the `valley shape` scalars in carve_river and guided_rolling_hills. (directional_blur instead gained a plain *spatial* `path` Curve — drawn in the 2D preview, overrides the angle when not the untouched default.)
...
- **Resample in premultiplied alpha**: any op that interpolates between pixels of an alpha-carrying image (2/4-channel) — resize, affine/warp remapping, edge cross-fades, thumbnails — must premultiply first, or fully transparent pixels bleed their hidden RGB into interpolated edges (white fringe around dark glyphs on transparent PNGs). Pattern: `FloatImage::premultiply_alpha()` → resample → `unpremultiply_alpha()`, or use `resize_premultiplied`/`resize_fit_premultiplied` (all in `float_image.rs`; no-ops for 1/3-channel). Guidance/displacement/intensity map inputs are data, not imagery — never premultiply them; an edge-fill color must join the source's space (premultiply the fill too, see `transform.rs`). Degenerate-param passthroughs return the original `Arc` untouched, skipping the round-trip.
...
- **Concurrent-edit safety**: `Graph.last_synced_mtime` + `Graph.last_synced_hash` (both set on load/save) + `disk_conflicts()` make auto-save detect an external rewrite instead of clobbering it — the engine pauses saving, sends `GraphChangedMessage::FileConflict`, and waits for `ChangeGraphMessage::ResolveFileConflict { keep_ours }` (true = overwrite; false = `GraphCleared` + reload from disk). Detection is two-stage: the cheap mtime check runs every time; a newer mtime then compares file *content* against the hash, because cloud-sync clients (Google Drive/OneDrive/Dropbox) re-stamp a file's mtime after uploading our own save — same bytes + newer mtime is treated as a sync touch (baseline refreshed, no conflict), different bytes is a real conflict. Subgraph child load failures / unknown child nodes surface as `NodeChangedMessage::Error` on the parent subgraph node
...
- `curve/` — curve→number measurements (output-type rule, mirrors `numbers/image/`): length, point_count (control points, matches the overlay), bounds (x/y/w/h), centroid (arc-length-weighted), area (absolute + signed; open curves implicitly closed, positive = clockwise on screen since y-down), sample_point (t → x, y, tangent angle°). All values in normalized [0,1] curve units; degenerate curves emit documented fallbacks, never errors.
...
### curves/
All curve-space params are normalized [0,1] units; length-like params (tolerance, spacing, jitter, offset distance, corner radius) are authored as px@1024 and divided by 1024 into normalized units — a unit convention only, not `scale_to_resolution` (curves have no raster size). Shared polyline helpers live in `curves/common.rs` (`flatten_f64`, `linear_curve`, `resample`, `rdp_decimate`, `cumulative_arc`, `vertex_tangent`, `MAX_OUTPUT_POINTS`=4000 cap on emitted curves — outputs persist in graph saves); `Curve` itself has `length`/`sample`/`bounds`/`signed_area`/`tangent_at`/`rasterize`. Nodes with <2-point curve inputs pass through/fall back, never error.
- `inputs/` — curve (emits a `Value::Curve`; drawn via the Preview2D overlay — see the Curve bullet in Key Conventions)
- `generators/` — parametric curve sources: ellipse + arc (few-anchor Bezier with exact mirrored-handle tangents, hand-editable downstream), polygon + star (exact-vertex Linear), spiral, superellipse, lissajous, wave (sine/zigzag/sawtooth between two endpoints) as dense Linear, and seeded (seed-first) random_walk + fractal_line (midpoint displacement, exact endpoints — natural meander/carve_river feeder)
- `modify/` — Curve→Curve. Structure-preserving (keep interpolation/closed, handles transformed as vectors): transform (rotate/scale about arc-length centroid + translate), mirror, reverse (negates handle vectors — exact double-apply identity). Flattening (emit Linear via `common`): smooth (chaikin/laplacian on control points), simplify (RDP), resample (spacing/count), jitter (seeded normal-displacement), offset (parallel curve; resample→normal offset→relax heuristic, self-intersections not removed), trim (arc-length t₀..t₁, closed curves cut open at arc 0), round_corners (quadratic fillets on control points)
- `combine/` — join (concat; auto-orient reverses b if closer end-to-end; same interpolation kept, mixed flattens to Linear), morph (resample both to max count, pointwise lerp; closed = a && b)
- `from_image/` — trace_contour (image→Curve: luma threshold → marching squares with edge interpolation + center-sample saddle disambiguation → longest contour → RDP; closed flag from contour topology; empty/full mask → `Curve::default()`)
...
### images/
- `inputs/` — file, from_folder (one image from a folder by index), url, clipboard, color, gradient, text, constant (number-driven solid grayscale fill)
...
- `adjustments/` — brighten, contrast, exposure (Photoshop-style exposure/offset/gamma), levels, auto_levels, curves (Photoshop-style spline tone curve; see the Curve bullet's tone-curve exception), grayscale, black_white (per-colour-range weighted monochrome + tint), invert, posterize, saturation, vibrance (uniform saturation + (1−s)-weighted vibrance), hue_rotate, hsl, threshold, vignette, white_balance, photo_filter (colour tint + density + preserve-luminosity), color_balance, selective_color, color_to_mask, replace_color, frequency_split, dither, gradient_map, gradient_dynamic, color_match, color_lookup (Adobe .cube 1D/3D LUT via trilinear interpolation), clarity (midtone local-contrast unsharp), dehaze (heuristic dark-channel-prior haze removal), grain (seeded film grain), distance, histogram_scan, histogram_range, histogram_select (shared `smoothstep`/HSL helpers live in `adjustments/common.rs`)
...
- `shapes/` — rectangle, ellipse, circle, polygon, star, line, cone, pyramid, paraboloid, rasterize_curve (Curve → grayscale mask: AA stroke with px@1024 `stroke width`/`feather`, even-odd fill when closed; feeds e.g. carve_river's river mask), curve_distance_field (Curve → falloff mask via `distance_field_labeled`; normalize/invert flags), curve_gradient (paints arc-fraction t outward from the nearest curve point — river flow/depth ramps pairing with meander's centerline; optional max-distance cutoff). Curve→image nodes output raw linear 1-channel like rasterize_curve.
- `patterns/` — brick, hexagonal, weave, tile_sampler, tile_generator, splatter, flood_fill, flood_fill_mapper, scatter_on_curve (stamps a pattern image along a curve at arc-length spacing, optional tangent alignment + seeded scale/rotation/along/across jitter; shares the `draw_stamp` max-blend compositor in `patterns/mod.rs` with splatter — nearest-sampled, extracted behavior-identical)
...
- `noise/` — 47 generators in subdirectories mirroring the node-menu subcategories (`voronoi_common.rs` at the noise root is a shared helper, not a node; `pixel_hash`/`periodic_perlin_2d`/`periodic_value_2d`/`build_perm_tables` in `noise/mod.rs` are shared)
  ...
  - `cellular/` — worley_distance, worley_value, voronoise, voronoi_crack, crystal, scales, craters, rolling_hills (Hann-kernel hill splatting; peakiness exponent reshapes the profile, optional `profile` tone curve remaps the dome on top of it — x = dome height, must return 0 at x=0 or hills get rim cliffs — merge slider blends tallest-wins→sum overlap composition)
