...
- `app/crates/mangler_gui/src/libraries/` — Libraries panel: named links to on-disk folders of `.mangle.json` graphs (local or network share; links persisted as `AppConfig::libraries`, content rescanned each session). `library_scanner.rs` polls all roots on a 2s background thread (no file-watcher — polling works on network shares); `libraries_state.rs` is app-global state on the GUI `App`, threaded through `render_tree` into `Program::show_panel`; panel actions that need the programs map (open/create graph → new tab) queue as `LibraryAction`s drained by `App::handle_library_action`. Deletes go to the OS recycle bin via the `trash` crate; removing a library only unlinks it
