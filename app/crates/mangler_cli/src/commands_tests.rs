...
/// Render against a non-existent node ID returns a "not found" error.
#[tokio::test]
async fn cmd_render_unknown_node_returns_err() {
    let path = create_temp_graph("render_nonode");
    let result = cmd_render(path.clone(), "ghost".to_string(), false).await;
    let _ = std::fs::remove_file(&path);
    let msg = result.unwrap_err();
    assert!(msg.contains("not found"), "unexpected error: {msg}");
}

/// Render against a node that isn't a `video to file` op fails inside
/// `read_output_config` (the node has no `path` / `fps` / etc. inputs).
#[tokio::test]
async fn cmd_render_wrong_node_type_returns_err() {
    let path = create_temp_graph("render_wrongtype");
    cmd_add_node(
        path.clone(),
        "numbers/arithmetic/add".to_string(),
        Some("add1".to_string()),
    ...
    .await
    .unwrap();
    let result = cmd_render(path.clone(), "add1".to_string(), false).await;
    let _ = std::fs::remove_file(&path);
    assert!(result.is_err());
}

/// End-to-end render: build a full video-in → extract-frame → video-out
/// graph, render it, and check the output file was written.
#[tokio::test]
async fn cmd_render_produces_nonzero_mp4() {
    let fixture = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/tiny.mp4");

    let path = create_temp_graph("render_e2e");
    let out_mp4 = std::env::temp_dir()
        .join(format!("mangle_render_{}.mp4", std::process::id()));
    let _ = std::fs::remove_file(&out_mp4);

    cmd_add_node(path.clone(), "videos/input/video_from_file".into(), Some("src".into()), None, false).await.unwrap();
    cmd_add_node(path.clone(), "videos/transform/extract_frame_by_time".into(), Some("mid".into()), None, false).await.unwrap();
    cmd_add_node(path.clone(), "videos/output/video_to_file".into(), Some("out".into()), None, false).await.unwrap();

    // src.video → mid.video ; mid.image → out.image
    cmd_connect(path.clone(), "src:0".into(), "mid:0".into(), false).await.unwrap();
    cmd_connect(path.clone(), "mid:0".into(), "out:0".into(), false).await.unwrap();

    cmd_set_input(
        path.clone(),
        "src".into(),
        vec![0],
        vec![format!("path:{}", fixture.display())],
        false,
    )
    .unwrap();
    cmd_set_input(
    ...
    let result = cmd_render(path.clone(), "out".into(), false).await;
    let _ = std::fs::remove_file(&path);

    assert!(result.is_ok(), "render failed: {:?}", result);
    let meta = std::fs::metadata(&out_mp4).expect("output video missing");
    assert!(meta.len() > 0, "output video is empty");
    let _ = std::fs::remove_file(&out_mp4);
}
