...
async fn opaque_alpha_is_preserved() {
    ...
    // taken over colour channels only (1 − 1 = 0 on alpha would blank it), with
    // the source alpha carried straight through.
    let mut img = FloatImage::from_pixel(7, 7, 4, &[0.2, 0.2, 0.2, 1.0]);
