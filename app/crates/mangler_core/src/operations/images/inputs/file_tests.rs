...
#[tokio::test]
async fn test_file_input_heic() {
    // 64x64 flat-red single-item HEIC (losslessly encoded BT.601 limited
    // YCbCr (81, 90, 240) → sRGB (254, 0, 0)); generated by heif-oxide's
    // `generate_sample_heic` fixture test.
    let path = PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/test_data/flat_red_64.heic"));
    let (img, w, h) = load(path).await;
    assert_eq!((w, h), (64, 64));
    assert_eq!(img.channels(), 3, "no alpha auxiliary → 3 channels");
    for y in 0..64u32 {
        for x in 0..64u32 {
            let px = img.get_pixel(x, y);
            let expected = [254.0 / 255.0, 0.0, 0.0];
            for c in 0..3 {
                assert!(
                    (px[c] - expected[c]).abs() < 0.01,
                    "pixel ({}, {}) channel {}: got {}, expected {}",
                    x, y, c, px[c], expected[c]
                );
            }
