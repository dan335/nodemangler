...
fn make_inputs(
    ...
    vec![
        ...
        Input::new("curl".to_string(), Value::Decimal(curl), None, None),
    ]
}

/// Default inputs matching the operation defaults.
fn default_inputs(seed: i32, width: i32, height: i32) -> Vec<Input> {
    make_inputs(seed, width, height, 14.0, 0.55, 0.5, 0.5, 0.5)
}

#[tokio::test]
async fn test_settings() {
    let s = OpImageNoisePeeling::settings();
    assert_eq!(s.name, "peeling noise");
    assert_eq!(OpImageNoisePeeling::create_inputs().len(), 8);
    assert_eq!(OpImageNoisePeeling::create_outputs().len(), 1);
}

#[tokio::test]
async fn test_run_basic() {
    let mut inputs = default_inputs(1, 16, 16);
    let result = OpImageNoisePeeling::run(&mut inputs).await;
...
async fn test_correct_dimensions() {
    ...
    let result = OpImageNoisePeeling::run(&mut inputs).await.unwrap();
...
async fn test_deterministic() {
    let r1 = OpImageNoisePeeling::run(&mut default_inputs(7, 16, 16)).await.unwrap();
    let r2 = OpImageNoisePeeling::run(&mut default_inputs(7, 16, 16)).await.unwrap();
    match (&r1.responses[0].value, &r2.responses[0].value) {
        (Value::Image { data: d1, .. }, Value::Image { data: d2, .. }) => {
            assert_eq!(
                d1.pixels().collect::<Vec<_>>(),
                d2.pixels().collect::<Vec<_>>(),
                "peeling noise is not deterministic"
            );
        }
...
async fn test_different_seeds_differ() {
    let r1 = OpImageNoisePeeling::run(&mut default_inputs(1, 16, 16)).await.unwrap();
    let r2 = OpImageNoisePeeling::run(&mut default_inputs(42, 16, 16)).await.unwrap();
    match (&r1.responses[0].value, &r2.responses[0].value) {
        (Value::Image { data: d1, .. }, Value::Image { data: d2, .. }) => {
            assert_ne!(
                d1.pixels().collect::<Vec<_>>(),
