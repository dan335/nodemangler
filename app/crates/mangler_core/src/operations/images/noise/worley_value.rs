...
use super::voronoi_common::{NoiseWorleyDistanceFunction, cell_hash, compute_distance, grid_size_from_frequency, pixel_to_grid, wrap_cell};
...
/// distinct flat-colored regions separated by cell boundaries. Always tiles
...
impl OpImageNoiseWorleyValue {
    ...
    /// outputs that cell's random value. Cell coordinates wrap via `rem_euclid`
    /// for seamless tiling.
            ...
            (0..w).map(move |x| {
                ...
                for dy in -1..=1 {
                    for dx in -1..=1 {
                        ...
                        if dist < min_dist {
                            min_dist = dist;
                            nearest_value = cell_hash(nx, ny, seed_u32, 2);
                        }
                    }
                }

                let noise = (nearest_value as f32).clamp(0.0, 1.0);
