...
#[tokio::test]
async fn test_percentile_with_nan_pixel_does_not_panic() {
    ...
    let mut inputs = inputs_with(img, 50.0);
    let r = OpNumberImagePercentile::run(&mut inputs).await;
    assert!(r.is_ok(), "percentile should not panic on a NaN pixel");
}
