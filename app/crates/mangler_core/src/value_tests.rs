...
/// Lock the container+codec compatibility table. Reflects `VideoContainer::
/// supported_codecs`; adding a variant or changing a row must update this test
/// in the same commit.
#[test]
fn test_container_codec_matrix() {
    use VideoCodec::*;
    use VideoContainer::*;

    let matrix: &[(VideoContainer, &[VideoCodec])] = &[
        (Mp4,  &[H264, H265, Av1, Mpeg4]),
        (Mov,  &[H264, H265, Mpeg4, ProRes]),
        (Mkv,  &[H264, H265, Vp8, Vp9, Av1, Mpeg4, ProRes]),
        (WebM, &[Vp8, Vp9, Av1]),
        (Avi,  &[H264, Mpeg4]),
    ];

    // Every entry in `matrix` must be the literal supported_codecs slice, and
    // every (container, codec) pair outside the entry must be rejected by
    // is_supported_in. Catches both over-permissive and under-permissive drift.
    for (container, supported) in matrix {
        assert_eq!(
            container.supported_codecs(),
            *supported,
            "supported_codecs mismatch for {:?}",
            container
        );

        for codec in VideoCodec::types() {
            let expected = supported.contains(&codec);
            assert_eq!(
                codec.is_supported_in(*container),
                expected,
                "is_supported_in({:?}, {:?}) expected {}",
                codec,
                container,
                expected,
            );
...
#[test]
fn test_video_container_default_extension() {
    ...
    assert_eq!(VideoContainer::Avi.extension(), "avi");
}

// ── Video value type ─────────────────────────────────────────────────────

#[test]
fn test_video_ref_serde_round_trip() {
    let meta = VideoMeta {
        width: 1920,
        height: 1080,
        fps: 29.97,
    ...
    let r = VideoRef {
        path: PathBuf::from("D:\\clips\\intro.mp4"),
        meta,
        source_meta: meta,
        transforms: Vec::new(),
    };
    let json = serde_json::to_string(&r).expect("serialize");
    let back: VideoRef = serde_json::from_str(&json).expect("deserialize");
    assert_eq!(back, r, "VideoRef should survive a JSON round trip");
}

#[test]
fn test_value_video_fingerprint_differs_by_path() {
    let v1 = Value::Video(VideoRef {
        path: PathBuf::from("a.mp4"),
        meta: VideoMeta::default(),
        source_meta: VideoMeta::default(),
        transforms: Vec::new(),
    });
    let v2 = Value::Video(VideoRef {
    ...
    assert_ne!(
        v1.fingerprint(),
        v2.fingerprint(),
        "different paths must produce different fingerprints so graph \
         invalidation propagates"
    );
}

#[test]
fn test_value_video_value_type() {
    let v = Value::Video(VideoRef::default());
    assert_eq!(v.value_type(), ValueType::Video);
}

#[test]
fn test_value_video_converts_to_self() {
    let v = Value::Video(VideoRef::default());
    let converted = v.try_convert_to(ValueType::Video).expect("should convert");
    match converted {
        Value::Video(_) => {}
...
#[test]
fn test_value_video_does_not_convert_to_image() {
    let v = Value::Video(VideoRef::default());
    assert!(
        v.try_convert_to(ValueType::Image).is_err(),
        "Video should not silently convert to Image — must go through \
         an extract-frame op"
    );
}
