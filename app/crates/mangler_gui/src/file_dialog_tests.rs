...
/// `App` stamps the program id on at open time so the pick can be routed back
/// to the right tab many frames later.
#[test]
fn requests_carry_their_program_id_into_the_intent() {
    let intent = FileDialogRequest::SubgraphPath {
        node_id: "node-7".to_owned(),
    }
    .into_intent(Some("prog-1".to_owned()));
    ...
    match intent {
        FileDialogIntent::SubgraphPath {
            ...
            assert_eq!(program_id, "prog-1");
            assert_eq!(node_id, "node-7");
        }
        other => panic!("expected SubgraphPath, got {other:?}"),
    }

    let intent = input_path(&["png"], None, None, None).into_intent(Some("prog-2".to_owned()));
    match intent {
...
/// The app-level requests don't belong to a tab and must survive having none.
#[test]
fn app_level_requests_need_no_program_id() {
    assert!(matches!(
        FileDialogRequest::AddLibrary.into_intent(None),
        FileDialogIntent::AddLibrary
    ));
    assert!(matches!(
        FileDialogRequest::OpenGraph.into_intent(None),
        FileDialogIntent::OpenGraph
    ));
}
