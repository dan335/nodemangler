...
impl ImageResize {
    pub fn run(&mut self, inputs: &[Input], outputs: &mut [Output]) -> Duration {
        ...
        let Value::Integer(mut width) = inputs[1].value else { panic!("not suported")};
        let Value::Integer(mut height) = inputs[2].value else { panic!("not suported")};
        ...
        let dynamic_image = match &inputs[0].value {
        ...
        outputs[0].value = match inputs[0].value.clone().value_type() {
