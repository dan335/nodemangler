...
impl OpColorInputHsla {
    ...
    pub async fn run(inputs: &Vec<Input>) -> Result<OperationResponse, OperationError> {
        ...
        let Ok(Value::Decimal(lightness)) = inputs[2].value.try_convert_to(ValueType::Decimal) else { return Err(OperationError { message: "Unable to convert to integer.".to_string() })};
