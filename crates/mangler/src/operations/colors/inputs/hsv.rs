...
impl OpColorInputHsva {
    ...
    pub async fn run(inputs: &mut Vec<Input>) -> Result<OperationResponse, OperationError> {
        ...
        let value_converted = inputs[2].value.try_convert_to(ValueType::Decimal);
        ...
        if value_converted.is_err() { input_errors.push((0, value_converted.as_ref().err().unwrap().message.clone())); }
        ...
        let Ok(Value::Decimal(value)) = value_converted else { return Err(OperationError { input_errors, node_error: Some("Error converting.".to_string()) }); };
