...
impl OpImageInputUrl {
    ...
    pub async fn run(inputs: &mut Vec<Input>) -> Result<OperationResponse, OperationError> {
        ...
        // convert inputs
        let url_converted = inputs[0].value.try_convert_to(ValueType::String);

        // gather errors
        if url_converted.is_err() { input_errors.push((0, url_converted.as_ref().err().unwrap().message.clone())); }

        // return if error
        if input_errors.len() > 0 { return Err(OperationError { input_errors, node_error: None }); }

        // get values
        let Ok(Value::String(url)) = url_converted else { return Err(OperationError { input_errors, node_error: Some("Error converting.".to_string()) }); };
