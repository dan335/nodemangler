                ...
                if nodes.contains_key(node_id) {
                    ...
                    if let Some(node) = p {
                        if &node.inputs.len() > input_index {
                            let converted = self
                                .value
                                .convert_to(node.inputs[*input_index].value.clone().value_type());
                            match converted {
                                Ok(converted_value) => {
                                    node.inputs[*input_index].value = converted_value;
                                }
                                Err(_) => {
                            ...
                            //         let converted =
                            //             DynamicImage::ImageLuma8(o.clone()).to_rgba32f();
                            //         node.inputs[*input_index].value =
                            //             Value::ImageRgba32F(converted);
                            //     }
